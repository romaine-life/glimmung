...
// latestHotSwapEntryForJob returns the most recent apply_hot_swap history entry
// recorded against job, and whether one exists. The initial "running" entry and
// the finalizer's terminal entry share a job_name, so the latest is the
...
func latestHotSwapEntryForJob(lease Lease, job string) (TestSlotHotSwapHistoryEntry, bool) {
	...
	for _, e := range hotSwapHistoryEntries(lease) {
		if e.Operation != "apply_hot_swap" || hotSwapEntryJobName(e) != job {
			continue
		}
		latest = e
		found = true
...
func isTerminalHotSwapStatus(status string) bool {
	switch status {
	case "persisted", "build_failed", "swap_failed", "timeout":
		return true
	}
	return false
}
