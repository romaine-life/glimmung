...
func deployImageToTestSlot(store ReadStore, preparer TestSlotPreparer, minter RunnerGitHubTokenMinter, performer deployImagePerformer, resolveRef refResolver, resolveImage testSlotImageResolver) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.EqualFold(strings.TrimSpace(req.ImageSource), "chart") {
			...
			if err != nil {
				// Terminal: any commit that cannot be resolved to a CI image is a
				// 422. The provisioning gate only deploys once the registry has
				// reported the image (tank-operator's durable ci_image_available
				// signal), so reaching here means the image is genuinely absent —
				// there is no in-flight-build window left to retry.
