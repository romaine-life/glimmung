...
func newHandlerWithReconcilers(settings Settings, store ReadStore, authResolver AuthResolver, ghClient any, workloadIdentities RunnerWorkloadIdentityReconciler, managedOrigins ManagedOriginReconciler, runLauncher RunLauncher, artifactStores ...ArtifactStore) http.Handler {
	...
	mux.HandleFunc("GET /v1/reviews", listReviews(store))
	mux.HandleFunc("GET /v1/projects/{project}/issues/{issue_number}/review", issueReviewDetail(store))
	mux.Handle("POST /v1/reviews", requireAdmin(adminAuthenticator, http.HandlerFunc(createReview(store))))
	...
	mux.Handle("PUT /v1/workflows/{project}/{name}/control-pins/{target}", requireAdmin(adminAuthenticator, http.HandlerFunc(pinWorkflowControl(store))))
	mux.Handle("DELETE /v1/workflows/{project}/{name}/control-pins/{target}", requireAdmin(adminAuthenticator, http.HandlerFunc(unpinWorkflowControl(store))))
	...
	mux.Handle("POST /v1/projects/{project}/issues/{issue_number}/runs/{run_number}/review/preview", requireAdmin(adminAuthenticator, http.HandlerFunc(previewRunReviewByNumber(store, artifactStore))))
	mux.Handle("POST /v1/projects/{project}/issues/{issue_number}/runs/{run_number}/cycles/{cycle_number}/review/preview", requireAdmin(adminAuthenticator, http.HandlerFunc(previewRunCycleReviewByNumber(store, artifactStore))))
	mux.Handle("POST /v1/projects/{project}/issues/{issue_number}/runs/{run_number}/review/merge", requireAdmin(adminAuthenticator, http.HandlerFunc(mergeRunReviewByNumber(store, prClient))))
	mux.Handle("POST /v1/projects/{project}/issues/{issue_number}/runs/{run_number}/cycles/{cycle_number}/review/merge", requireAdmin(adminAuthenticator, http.HandlerFunc(mergeRunCycleReviewByNumber(store, prClient))))
	...
	mux.HandleFunc("POST /v1/run-callbacks/{callback_token}/run/pr-review", runnerPRReviewByCallbackToken(store, prClient, artifactStore))
	...
	if dl, ok := runLauncher.(imageToSlotDeployer); ok {
		deployPerformer = func(ctx context.Context, lease Lease, project Project, verifiedRef, image, imageValueKey string) error {
			return dl.DeployImageToSlot(ctx, lease, project, runnerTokenMinter, verifiedRef, image, imageValueKey)
		}
	}
