...
Glimmung runner agent jobs do not own provider OAuth refresh chains.
...
- `codex-api-proxy` fronts `api.openai.com` for current Codex Responses API
  ...
  calls.
- Runner jobs write placeholder provider credentials with
  `managed-by-glimmung`.
- Runner jobs route provider hostnames to the proxy Service ClusterIPs through
  pod `hostAliases`.
...
proxy namespace. Runner jobs must never mount those Secrets. A runner job that
...
for the real provider hostnames. The run launcher mounts the proxy CA into
...
- Test slots and runner jobs do not create provider proxies with write access
...
The retired path is mounting real provider credential Secrets into runner jobs
and copying them into `$HOME/.codex` or `$HOME/.claude`. Tests pin that runner
Job manifests do not contain the old `codex-credentials` volume or
`/etc/codex-creds` mount, and the runner writes placeholder Codex auth
